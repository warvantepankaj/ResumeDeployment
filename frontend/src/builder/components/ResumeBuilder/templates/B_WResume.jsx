...
export function B_WResume({ resumeData = {} }) {
  const {
    ...
    languages = [], // Destructuring the new top-level languages array
    certificate = [],
    achievements = [],
  } = resumeData;
  ...
  // Safely check for soft skills
  const hasSoftSkills = skills.soft?.length > 0;
  const displaySoftSkills = skills.soft?.length > 0 ? skills.soft : [];
  
  const showSkillsSection =
    hasNewTechSkills || hasLegacyTechSkills || hasSoftSkills;

  // Support both old skills.languages and new top-level languages array
  const displayLanguages = skills.languages?.length > 0 ? skills.languages : [];
  const hasLanguages = displayLanguages.length > 0;
  console.log(skills.languages,"hccbdcuuwcwc");
  ...
  return (
    ...
    >
      <div className="bg-white shadow-xl min-h-[1123px] min-w-[794px] flex border border-gray-300 dark:border-gray-600">
        {/* ================= LEFT SIDEBAR ================= */}
        <div className="w-[35%] bg-[#374151]  text-gray-200 dark:text-gray-300 px-4 pt-4 flex flex-col gap-6">
          {/* Profile Image */}
            ...
            <div className="flex flex-col gap-2 font-medium mt-2">
              ...
              {personalInfo.email && (
                <div className="flex gap-2 items-center">
                  <MdOutlineAlternateEmail />
                  <p className="text-sm">
                    {personalInfo.email.length > 20
                      ? personalInfo.email.split(18) + ".."
                      : personalInfo.email}
                  </p>
                </div>
              )}
          ...
          {education.length > 0 && (
            <div className="w-full">
              ...
              {education.map((edu) => (
                <div key={edu.id} className="font-semibold text-sm mb-3 pl-1">
                  {edu.startDate && (
                  ...
                  <ul className="list-disc ml-5 mt-1 inline-block">
                    {edu?.degree && (
                      <li className="text-[13px] font-normal leading-tight">
                        {edu?.degree}
                      </li>
                    )}
                    {edu.scoreValue && (
          ...
          {hasSoftSkills && (
            <div>
              ...
              <ul className="text-[13px] pl-1 ml-5">
                {displaySoftSkills.map((skill, indx) => (
                  <li key={indx} className="list-disc">
                    {skill}
                  </li>
                ))}
          ...
          {hasLanguages && (
            <div>
              ...
              <ul className="text-[13px] pl-1 ml-5">
                {displayLanguages.map((language, indx) => (
                  <li key={indx} className="list-disc">
                    {language}
                  </li>
                ))}
        ...
        <div className="w-[68%] px-6 pt-5 bg-gray-50 dark:bg-gray-300">
            ...
            <div className="mb-1 mt-3">
              ...
              {projects.map((proj) => (
                ...
                >
                  ...
                  {/* Description */}
                  <div className="text-[11.5px] leading-tight text-gray-600 ">
                    {proj.description && (
                      <ul className="list-disc leading-4 text-[12px] dark:text-gray-600 pl-5">
                        {proj.description
                          .split(/(?<=\.)\s+(?=[A-Z])/g)
          ...
          {/* Technical Skills */}
          {hasNewTechSkills || hasLegacyTechSkills ? (
            <div className="mb-2 mt-3">
              <h2 className="font-semibold text-lg border-b border-gray-400 pb-1 mb-2 text-gray-700">
                TECHNICAL SKILLS
              </h2>
              ...
              <div className="text-[12px] pl-3">
                ...
                {hasLegacyTechSkills && (
                  <p>
                    <span className="text-[13px] inline-block pr-1 font-semibold text-gray-800">
                      Technical Skills:{" "}
                    </span>
                    {techSkills.join(", ")}
                  </p>
                ...
                {!isTechArray &&
                  techSkills.programmingLanguages?.length > 0 && (
                    <p className="text-gray-600">
                      <span className="text-[13px] inline-block pr-1 font-semibold text-gray-700">
